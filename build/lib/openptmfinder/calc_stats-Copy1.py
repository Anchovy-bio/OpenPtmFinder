...
def bayesian_site_aggregation(group, global_batch, method = 'aggregate',eps=1e-8, 
...
def build_expression_noagg(df, method="aggregate"):
    """
...
def compute_batch_global_prior(batch_df, method="median"):
    """
...
def statistics(df, min_group_for_stats, method = 'aggregate', variance_filter=0.25):
    """
    ...
    channel_cols = [c for c in df.columns if c.endswith("_norm")]
    df.dropna(subset=channel_cols, how='all', inplace=True, ignore_index=True)
    ...
    if method == 'aggregate':
        ...
        df_site = (
            df.groupby(["protein", "position_in_protein", "batch"])
                .apply(lambda g: bayesian_site_aggregation(g, global_prior, method='aggregate'))
                .reset_index()
        )
        ...
        global_prior = {}
        for batch in df['batch'].unique():
            var_ = compute_batch_global_prior(df[df['batch']==batch], method = 'median')
            global_prior[batch] = var_
        ...
        df_site = (
            df.groupby(["batch", "peptide"])
                .apply(lambda g: bayesian_site_aggregation(g, global_prior, method='median'))
                .reset_index()
        )
